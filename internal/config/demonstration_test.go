...
func TestErrorReportingDemonstration(t *testing.T) {
	...
	t.Run("GameStartErrorContext", func(t *testing.T) {
		...
		target := "C:\\Program Files (x86)\\Steam\\steamapps\\common\\AdventureGame\\GameName.exe"

		// Old generic error
		oldError := "failed to start game: no such file or directory"

		// New contextual error (simulated format from our improvements)
		newError := "failed to start game 'adventure' (mode: DirectPath, target: C:\\Program Files (x86)\\Steam\\steamapps\\common\\AdventureGame\\GameName.exe): failed to start process 'C:\\Program Files (x86)\\Steam\\steamapps\\common\\AdventureGame\\GameName.exe': no such file or directory"
