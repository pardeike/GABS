...
type blockingTestConnector struct{}

func (blockingTestConnector) AttemptConnection(ctx context.Context, gameID string, port int, token string) error {
	<-ctx.Done()
...
func TestSerializedStarterStopsWaitingWhenProcessExitsDuringGABPConnect(t *testing.T) {
	...
	start := time.Now()
	result := starter.StartWithVerification(controller, blockingTestConnector{}, spec.GameId, 0, "")
	...
	if result.GABPConnected {
		t.Fatal("expected GABP connection to fail when the process exits")
	}
	if !result.ProcessExitedDuringGABP {
		t.Fatal("expected the starter to notice that the process exited during GABP connection")
	}
	if result.GameStillRunning {
		t.Fatal("process should not still be running after exit")
	}
	if result.GABPConnectError == nil {
		t.Fatal("expected a GABP connection error")
	}
	if !strings.Contains(result.GABPConnectError.Error(), "exited before GABP became available") {
		t.Fatalf("unexpected GABP error: %v", result.GABPConnectError)
	}
